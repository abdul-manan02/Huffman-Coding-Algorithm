...
unoptimizedTree2 :: unoptimizedTree2()
{
	root = NULL;
	nodes = NULL;
	codes = NULL;
...
unoptimizedTree2 :: unoptimizedTree2(int Size)
{
	root = NULL;
	size = Size;
...
void unoptimizedTree2 :: buildHuffmanTree(char ch[], int freq[])
{
	node* left = NULL;
	node* right = NULL;
	node* parent = NULL;

	priority_queue < node*, vector<node*>, condition2 > maxQueue;
				// vector & condition class have been implemented to make 
				//		max-priority queue for user-defined data type
	...
	for (int i = 0; i < size; i++)
	{
		...
		// nodes assigned in descending order wrt their frequencies
		maxQueue.push(&nodes[i]);
	}
...
void unoptimizedTree2 :: calculateCodes(node* Root, string code)
{
...
void unoptimizedTree2 :: printCodes()
{
	cout << "\n";
	cout << "\t\t\t\t\t C O D E S \n";
...
void unoptimizedTree2 :: encodeDecode(char data[])
{
...
void unoptimizedTree2 :: bitRate_compressionRatio()
{
	float bitRate = 0;
	float temp = 0;
	float ratio = 0;
